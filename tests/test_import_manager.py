...
class TestImporterManager(TestCase):
    ...
    def test_all_objects_returned(self):
        """Tests that ImporterManager works for a model without dependencies
        """
    ...
    def test_many_to_many_structure(self):
        ...
        manager.update_kvs(field_name='username',value=self.usernames[0],row=0)
        manager.update_kvs(field_name='username',value=self.usernames[1],row=0,col=1)

        manager.update_kvs(field_name='username',value=self.usernames[2],row=1)
        manager.update_kvs(field_name='username',value=self.usernames[3],row=1,col=1)

        manager.get_available_rows()

        for i in range(2):
            objs = manager.get_objs_and_meta(i)
            for j in range(2):
